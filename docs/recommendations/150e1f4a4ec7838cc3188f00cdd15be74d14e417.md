...
<body>
  <p>
  <i class="footer">This page was last updated on 2025-01-21 10:42:35 UTC</i>
  </p>
  ...
  <tbody>
        ...
        <tr id="1634 The purpose of this study was to develop a computational strategy to identify oncogene-driven gene expression networks in cancer cells. By comparing breast epithelial cells, where the proto-oncogene HER-2 participates in a normal cell signaling network, with breast cancer cells where HER-2 is the oncogene driving transformed phenotypes, we tested the hypothesis that over expression of the HER-2 oncogene results in changes in the gene expression network regulated by HER-2, and results in a new network comprised mainly of cancer cell-specific controller or hub genes. To begin, we obtained whole-genome expression data by microarray analysis at 16 points over 45 hours where HER-2 signaling was blocked by treatment with the HER-2-specific inhibitor CP-724714. A dynamic linear finite difference model that shows the influence of one gene expression level change on every other gene was used to assign gene connectivity values, which shape phenomenological networks. Gene expression and connectivity values were determined for 2500 genes from the spontaneously immortalized MCF10A cell line, and for two isogenic cell lines transformed by over expressing HER-2 that exhibit successive states of growth factor independence. Comparative analysis showed that HER-2-driven oncogenic transformation alters gene connectivity and identified cancer cell-specific hub genes linked to the oncogene that were unique to each cell line. Application of these findings to map interaction networks based on annotated resources using Genomatix and Ingenuity Systems software identified novel interactions between HER-2 and specific signaling network modules. Results show that, compared to a normal cell, HER-2 transformed cells incorporate different genes into the networks. Also, the interactions among some genes that occur in networks from more than one cell-type have different interactions in the three cell lines. Namely, TGFB1 and DKK1 are hubs in phenomenological networks specific to HER-2 over expression, yet show differences in their expression dynamics and network connectivity in individual transformed cell lines. NFKBIA was found to be a hub gene only in the HER-2 transformed cells that are completely growth factor independent. The connectivity of these genes in the interaction networks of our isogenic cell lines implicates WNT, TGF-beta, and NFKB signaling pathways in the HER-2 oncogene transformation of breast epithelial cells. In summary, the investigative approach we describe resulted in the identification of oncogene-specific networks that can be linked to the expression of specific transformed phenotypes.">
          <td id="tag"><i class="material-icons">visibility_off</i></td>
